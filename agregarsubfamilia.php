...
if ($_SERVER['REQUEST_METHOD'] =='POST' ) {
	...
	if (empty($nombre)) {
			$errores .='<li>Por favor ingresa una Sub-Familia</li>';
	}else{
		...
		$statement = $conexion->prepare('SELECT	* FROM subfamilia WHERE subfamilia =:nombre LIMIT 1 ');
		...
		$statement = $conexion->prepare('INSERT INTO subfamilia 
			(idsubfamilia,subfamilia,usuarios_idusuarios) 
		...
		$resultado = $statement->fetch();
			...
			$agregado .='<li>La Sub-familia se a agregado</li>';
...
require 'views/agregarsubfamilia.view.php';
